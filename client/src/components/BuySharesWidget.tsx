...
interface BuySharesWidgetProps {
    data: {
        ticker?: string;
        quantity?: number;
        price?: number;
...
export default function BuySharesWidget({ data, onClose, onOutputChange, onOpenSettings }: BuySharesWidgetProps) {
    ...
    const handleBuy = async () => {
        ...
        try {
            const res = await fetch('http://localhost:8261/api/portfolio/update', {
                ...
                body: JSON.stringify({ action: 'add', ticker: ticker.toUpperCase(), shares: quantity, cost_basis: price || 0 }),
            });
            ...
            if (json.error) {
                ...
                setStatus('success');
                setMessage(json.success || `Bought ${quantity} shares of ${ticker.toUpperCase()}`);
                onOutputChange?.({ status: 'bought', ticker: ticker.toUpperCase() });
            }
        } catch (err: any) {
            setStatus('error');
    ...
    return (
        <div className="glass-panel" style={{ height: '100%', display: 'flex', flexDirection: 'column', position: 'relative' }}>
            ...
            <div className="drag-handle" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', cursor: 'grab' }}>
                <h3 style={{ margin: 0, color: '#3fb950', display: 'flex', alignItems: 'center', gap: '8px', fontSize: '1rem' }}>
                    <ShoppingCart size={17} /> Buy Shares
                </h3>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', flex: 1 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                    ...
                    <div>
                        <label style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginBottom: '4px', display: 'block' }}>Cost Basis ($)</label>
                        <input
                ...
                <button
                    ...
                    style={{
                        ...
                        background: status === 'pending' ? 'rgba(63, 185, 80, 0.3)' : 'linear-gradient(135deg, #238636, #2ea043)',
                        color: '#fff', fontWeight: 700, fontSize: '0.95rem', marginTop: '4px',
                        transition: 'all 0.2s ease', boxShadow: '0 2px 8px rgba(63,185,80,0.25)',
                    }}
                    onMouseEnter={(e) => { if (status !== 'pending') (e.currentTarget.style.boxShadow = '0 4px 16px rgba(63,185,80,0.4)'); }}
                    onMouseLeave={(e) => { e.currentTarget.style.boxShadow = '0 2px 8px rgba(63,185,80,0.25)'; }}
                >
                    {status === 'pending' ? '⏳ Processing...' : '🛒 Buy Shares'}
                </button>
