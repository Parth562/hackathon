...
export default function ChartWidget({ data, onClose }: ChartWidgetProps) {
    ...
    // Detect if this is a generic data stream (e.g., from MathWidget out-series -> ChartWidget in-data)
    // where data is shaped roughly like `data: [{ time: "2024-01-01", value: 123 }, ...]`
    const isGenericSeries = !isComparison && !isPie && !isBar && Array.isArray(data.data) && data.data.length > 0 && typeof data.data[0] === 'object' && ('time' in data.data[0] || 'date' in data.data[0]);
            ...
            <div style={{ width: '100%' }}>
                ...
                <ResponsiveContainer width="100%" height={350}>
                    <LineChart data={data.data} margin={{ top: 10, right: 30, left: 20, bottom: 5 }}>
                        ...
                        <XAxis dataKey={data.data[0].time ? 'time' : 'date'} stroke="#8b949e" tick={{ fill: '#8b949e', fontSize: 12 }} />
                        <YAxis stroke="#8b949e" domain={['auto', 'auto']} tickFormatter={(val) => `${val.toFixed(2)}`} />
                        <Tooltip
                            contentStyle={{ backgroundColor: 'rgba(13, 17, 23, 0.9)', borderColor: 'rgba(48, 54, 61, 1)', borderRadius: '8px', color: '#fff' }}
                            itemStyle={{ color: '#c9d1d9' }}
                            labelStyle={{ color: '#8b949e' }}
                        />
                        <Line type="monotone" dataKey="value" stroke="var(--primary)" strokeWidth={3} dot={false} name="Result" />
                    </LineChart>
                </ResponsiveContainer>
            </div>
        );
    } else if (isPie && data.data) {
        chartContent = (
